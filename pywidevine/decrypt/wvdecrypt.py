...
class WvDecrypt(object):
    ...
    def __init__(self, PSSH,  server_cert_required=None, cert_b64=None):
        ...
        self.session = self.cdm.open_session(check_pssh(self.pssh),
                              deviceconfig.DeviceConfig(deviceconfig.device_space_l3))

        if server_cert_required:
            self.cdm.set_service_certificate(self.session, cert_b64)
